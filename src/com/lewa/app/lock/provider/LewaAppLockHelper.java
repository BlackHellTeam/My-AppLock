package com.lewa.app.lock.provider;
...
public class LewaAppLockHelper {
    private AppLockDBHelper appLockHelper;
    private static final String TABLE = "lewalock";
    private static final String COLUMN_APPID = "packname";
    private static final String COLUMN_VALUE = "value";

    public LewaAppLockHelper(Context context) {
    ...
    public List<String> getAllLockedPackName(String lockValue) {
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select packname from lewalock where value=?",
    ...
    public boolean findLockApp(String packName) {
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery(
                    "select packname from lewalock where packname=?",
                    new String[] { packName });
    ...
    public void delete(String packname) {
        SQLiteDatabase db = appLockHelper.getWritableDatabase();
        if (db.isOpen()) {
            db.execSQL("delete from lewalock where packname=?",
                    new Object[] { packname });
            db.close();
        }
    }
