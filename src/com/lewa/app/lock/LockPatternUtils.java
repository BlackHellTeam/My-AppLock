...
import com.lewa.app.lock.provider.AppLockData;
import com.lewa.app.lock.provider.PreferencesProvider;
...
public class LockPatternUtils {
    ...
    private static final String LEWA_LOCK_PATTERN_FILE = "applock.key";
    ...
    public LockPatternUtils(Context context) {
        ...
        Log.d("liuwenshuai","system data:"+getLewaLockPatternFilename());
    ...
    private String getLewaLockPatternFilename() {
        String dataSystemDirectory = Environment.getDataDirectory().getAbsolutePath();
        return dataSystemDirectory + SYSTEM_DIRECTORY + LEWA_LOCK_PATTERN_FILE;
    }
    ...
    public boolean savedLewaPatternExists() {
        return new File(getLewaLockPatternFilename()).length() > 0;
    }
    /**
     * Check to see if the user has stored a lock pattern.
     * 
    ...
    public void clearLewaAppLock() {
        saveLewaAppLockPattern(null);
    }
    /**
     ...
     *            Specifies if this is a fallback to biometric weak
     */
    public void saveLewaAppLockPattern(List<Cell> pattern) {
        // Compute the hash
        final byte[] hash = LockPatternUtils.patternToHash(pattern);
        Log.d("liuwenshuai", "saveLewaAppLockPattern->hash:" + hash);
        try {
            // Write the hash to file
            RandomAccessFile raf = new RandomAccessFile(getLewaLockPatternFilename(), "rwd");
            // Truncate the file if pattern is null, to clear the lock
            if (pattern == null) {
                raf.setLength(0);
            } else {
     ...
     * @return Whether the pattern matches the stored one.
     */
    public boolean checkLewaAppLockPattern(List<Cell> pattern) {
        try {
            // Read all the bytes from the file
            RandomAccessFile raf = new RandomAccessFile(getLewaLockPatternFilename(), "r");
            final byte[] stored = new byte[(int) raf.length()];
            int got = raf.read(stored, 0, stored.length);
            raf.close();
            if (got <= 0) {
